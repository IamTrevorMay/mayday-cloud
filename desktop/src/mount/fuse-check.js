...
function checkMacFuse() {
  ...
  try {
    const kext = _deps.execSync('kextstat 2>/dev/null | grep -i macfuse', { encoding: 'utf8', timeout: 5000 });
    if (kext.trim()) {
  ...
  if (_deps.existsSync('/Library/Filesystems/macfuse.fs') || _deps.existsSync('/usr/local/lib/libfuse.dylib')) {
    return { installed: true, name: 'macFUSE', installUrl: 'https://osxfuse.github.io/' };
  }

  // Check for FUSE-T (newer, userspace-based alternative)
  if (_deps.existsSync('/Library/Filesystems/fuse-t.fs') || _deps.existsSync('/usr/local/lib/libfuse-t.dylib')) {
    return { installed: true, name: 'FUSE-T', installUrl: 'https://www.fuse-t.org/' };
  }

  // Check via homebrew
  try {
    const brew = _deps.execSync('brew list --cask 2>/dev/null', { encoding: 'utf8', timeout: 10000 });
...
function checkWinFsp() {
  ...
  try {
    const reg = _deps.execSync('reg query "HKLM\\SOFTWARE\\WinFsp" /v InstallDir 2>nul', { encoding: 'utf8', timeout: 5000 });
...
function checkLinuxFuse() {
  ...
  try {
    if (_deps.existsSync('/dev/fuse')) {
      return { installed: true, name: 'FUSE', installUrl: null };
    }
    const modinfo = _deps.execSync('modinfo fuse 2>/dev/null', { encoding: 'utf8', timeout: 5000 });
...
module.exports = { checkFuse, checkMacFuse, checkWinFsp, checkLinuxFuse, _deps };
