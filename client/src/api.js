...
function uploadTus(config, localPath, remoteDirPath, callbacks = {}) {
  return new Promise((resolve, reject) => {
    ...
    upload.findPreviousUploads().then((prev) => {
      ...
      // started. The size lives on the PreviousUpload object (prev[0].size),
      // not in metadata (which holds only filename/targetPath) — the old read
      // of prevMeta.size was always undefined, so resume never triggered.
      const resumable = pickResumableUpload(prev, fileSize);
      if (resumable) {
        logger.info(`Resuming previous upload for ${fileName}`);
        upload.resumeUpload(resumable);
      } else {
        if (prev.length > 0) logger.info(`File changed since last upload attempt for ${fileName}, starting fresh`);
        upload.start();
      }
    });
  });
